...
const DEMO = (() => {
  ...
  function makeTelemetry(seed, style){
    ...
    const n = 800, ch = {speed:[],throttle:[],brake:[],clutch:[],steer:[],gear:[],rpm:[],lapdist:[],
      lataccel:[],lonaccel:[],vertaccel:[],yawrate:[],velx:[],vely:[],yaw:[],heave_f:[],heave_r:[],rh_lf:[],rh_rf:[],rh_lr:[],rh_rr:[],
    ...
    for (let i=0;i<n;i++){
      ...
      ch.velx.push(+speed.toFixed(3)); ch.vely.push(0);
      ch.yaw.push(+((a + Math.PI/2) % (2*Math.PI)).toFixed(4));
      ch.heave_f.push(+(0.021 + 0.005*Math.sin(3*a)).toFixed(4));
      ch.heave_r.push(+(0.024 + 0.005*Math.sin(3*a+1)).toFixed(4));
